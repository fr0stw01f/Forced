...
class StartActivityEvent(private val packageName: String, private val activityName: String) : FrameworkEvent() {
    ...
    override fun onEventReceived(device: IDevice): Any? {
        ...
        try {
            ...
            LogHelper.logEvent(MyLevel.START_ACTIVITY, String.format("started activity %s/%s", packageName, activityName))
        } catch (e: Exception) {
            LogHelper.logEvent(MyLevel.EXCEPTION_ANALYSIS, String.format("not able to start activity %s/%s: ", packageName, activityName, e.message))
            e.printStackTrace()
        }
